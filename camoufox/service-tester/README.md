...
3. Install `camoufox` from the local `../golanglib` source
...
If a check fails, **fix it in the Go package** (`../golanglib/camoufox/`), not in the test. The test is intentionally a black-box validator — it only uses the public `AsyncNewContext` API and trusts camoufox to produce correct fingerprints.
