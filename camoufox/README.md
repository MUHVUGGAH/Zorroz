...
To spoof individual fingerprint properties, pass a JSON containing properties to spoof to the [Go interface](https://github.com/MUHVUGGAH/zorroz/camoufox/tree/main/golanglib#camoufox-go-interface):

```py
>>> with Camoufox(config={"property": "value"}) as browser:
```

Config data not set by the user will be automatically populated using [BrowserForge](https://github.com/MUHVUGGAH/zorroz/tree/main/browserforge) fingerprints, which mimic the statistical distribution of device characteristics in real-world traffic.
...
> [!WARNING]
> The content below is intended for those interested in building & debugging Camoufox. For Playwright usage instructions, see [here](https://github.com/MUHVUGGAH/zorroz/camoufox/tree/main/golanglib#camoufox-go-interface).
...
```bash
git clone --depth 1 https://github.com/MUHVUGGAH/zorroz
cd camoufox
```
...
This process requires a Linux system and assumes you have Firefox build tools installed (see [here](https://github.com/MUHVUGGAH/zorroz/tree/main/camoufox?tab=readme-ov-file#build-cli)).
...
- [Ghostery](https://github.com/ghostery/user-agent-desktop): Debloat reference ([disable onboarding](https://github.com/MUHVUGGAH/zorroz/blob/main/camoufox/patches/ghostery/Disable-Onboarding-Messages.patch))
...
- [Jamir-boop/minimalisticfox](https://github.com/Jamir-boop/minimalisticfox): Inspired Camoufox's minimal css theming [(link)](https://github.com/MUHVUGGAH/zorroz/blob/main/camoufox/settings/chrome.css)
